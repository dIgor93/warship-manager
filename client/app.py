...
from config import ENTITY_PATH, RPS
from core.game import main_game
from client.data_bus import DataBus
...
async def startup_event():
    ...
    loop.create_task(main_game(app.state.data_bus))


async def response_for_all():
    last = time.time()
    while True:
        message = app.state.data_bus.get_message()
        curr = time.time()
